...
func initSymbolData(s pb.Symbol, cache *XCache) *symbolData {
	sd := &symbolData{}
	ti, err := cache.xdb.GetTradingInstrument(&s)
	if err != nil {
		return nil
	}
	sd.ti = *ti
	sd.Symbol = s
	sd.cache = cache
	tick, err := sd.cache.xdb.GetTick(&s)
	if err == nil {
		sd.tick = tick
	} else {
	...
	for i := 1; i < 100; i++ {
		...
		ks := cache.xdb.RGetKlineSeries(&s, pb.PeriodType(i), 0, nowT, int64(cache.maxKlineLen))
		if ks == nil {
	...
	ts := cache.xdb.GetLastTickSerires(&s)
	if ts != nil {
		sd.dayTS = *ts
...
func (cache *XCache) loader() {
	l := cache.xdb.TradingInstrumentList()
	log.Println("初始化载入", len(l))
	for i := range l {
		cache.getSymbolData(l[i].Symbol)
	}
}
...
// func (cache *XCache) findSymbolData(s *pb.Symbol) *symbolData {
// 	cache.sdMapLock.RLock()
// 	v, ok := cache.sdMap[*s]
// 	cache.sdMapLock.RUnlock()
// 	if !ok {
...
func (cache *XCache) getLastTickSerires(s *pb.Symbol) *pb.TickSeries {
	return cache.xdb.GetLastTickSerires(s)
}

func (cache *XCache) rGetKlineSeries(s *pb.Symbol, period pb.PeriodType, startTime, endTime, lenLimit int64) *pb.KlineSeries {
	return cache.xdb.RGetKlineSeries(s, period, startTime, endTime, lenLimit)
}
...
func (cache *XCache) unsubscribe(req *pb.ReqSubscribe, ch chan pb.MarketDataSnapshot) {
...
func (cache *XCache) setTradingInsrument(req *pb.ReqSetTradingInstrument) {
	for i := range req.List {
		ti := req.List[i]
		cache.xdb.SetTradingInstrument(ti)
