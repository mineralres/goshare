...
func makeSyncClient(address string) *SyncQuoteClient {
	return &SyncQuoteClient{address: address}
}

func (c *SyncQuoteClient) init() {
	connect := func() {
		c.Lock()
		defer c.Unlock()
		if c.ready {
...
func (c *SyncQuoteClient) read() (*tdxResponse, error) {
	conn := c.conn
